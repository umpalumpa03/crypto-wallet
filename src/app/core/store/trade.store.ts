...
export const TradeStore = signalStore(
  ...
  withMethods((store, http: HttpClient = inject(HttpClient), authService: AuthService = inject(AuthService)) => {
    ...
    return {
      ...
      async loadDepositConfigs(force: boolean = false): Promise<void> {
        ...
        try {
          const data: Record<string, DepositConfig> = await lastValueFrom(http.get<Record<string, DepositConfig>>(`${apiUrl}/deposit-info`));
